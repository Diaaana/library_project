...
public class InputParamValidator {
    ...
    public static boolean isValidateUserData(String surname, String name, String middleName, int age, String phone, String mail, String login, String password) {
        return isCheckStringData(surname, RegularExpressions.NAME_PATTERN) && isCheckStringData(name, RegularExpressions.NAME_PATTERN)
                && isCheckStringData(middleName, RegularExpressions.MIDDLE_NAME_PATTERN) && isCheckStringData(phone, RegularExpressions.PHONE_NUMBER_PATTERN)
                ...
                && login.length() >= 5 && password.length() >= 6;
    }
    ...
    public static boolean isValidateLibrarianData(String surname, String name, String middleName, String login) {
        return isCheckStringData(surname, RegularExpressions.NAME_PATTERN) && isCheckStringData(name, RegularExpressions.NAME_PATTERN)
                && isCheckStringData(middleName, RegularExpressions.MIDDLE_NAME_PATTERN) && isCheckStringData(login, RegularExpressions.LOGIN_PATTERN)
                && login.length() >= 5;
    }
