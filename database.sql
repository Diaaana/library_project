...
CREATE TABLE IF NOT EXISTS `library`.`books` (
  ...
  `isbn` VARCHAR(13) CHARACTER SET 'utf8' COLLATE 'utf8_danish_ci' NOT NULL COMMENT 'Поле хранит международный стандартный книжный номер (уникальный номер книжного издания, необходимый для распространения книги в торговых сетях и автоматизации работы с изданием). Данное поле является уникальным.',
  ...
  PRIMARY KEY (`id_book`),
  UNIQUE INDEX `isbn` (`isbn` ASC)  COMMENT 'Индекс является уникальным, так как каждая книга имеет свой международный стандартный книжный номер.',
...
CREATE TABLE IF NOT EXISTS `library`.`borrow_book` (
  ...
  `number_ticket` INT(11) NOT NULL COMMENT 'Поле хранит номер билета читателя. Является внешним ключом, предназначено для хранения значения первичного ключа таблицы \"readers\" с целью организации связи между этими таблицами.',
  ...
  INDEX `number_ticket_idx` (`number_ticket` ASC),
...
CREATE TABLE IF NOT EXISTS `library`.`orders` (
  ...
  INDEX `mber_ticket_idx` (`number_ticket` ASC),
